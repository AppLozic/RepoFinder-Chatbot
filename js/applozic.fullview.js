    ...
    MCK_LABELS = appOptions.labels;
      ...
      MCK_LABELS = optns.labels;
		...
		function MckMessageService() {
			...
			$applozic(d).on("click", ".mck-msg-reply-button",function() {
				var msg= $applozic(this).html();
				...
				$applozic(".mck-msg-reply-button").removeClass("vis").addClass("n-vis");
				var msgKey = $applozic(this).parent().data("msgkey");
			...
			_this.sendMessage = function(messagePxy,callback) {
				$mck_msg_inner = mckMessageLayout.getMckMessageInner();
		...
		function MckMessageLayout() {
			...
			var markup = '<div name="message" class="bubble mck-m-b ${msgKeyExpr} ${msgFloatExpr} ${msgAvatorClassExpr}" data-msgdelivered="${msgDeliveredExpr}" data-msgsent="${msgSentExpr}" data-msgtype="${msgTypeExpr}" data-msgtime="${msgCreatedAtTime}" data-msgcontent="${replyIdExpr}" data-msgkey="${msgKeyExpr}" data-contact="${toExpr}"><div class="mck-clear"><div class="blk-lg-12"><div class="mck-msg-avator blk-lg-3">{{html msgImgExpr}}</div><div class="mck-msg-box ${msgClassExpr}">' + '<div class="${nameTextExpr} ${showNameExpr}">${msgNameExpr}</div><div class="mck-file-text notranslate mck-attachment n-vis" data-filemetakey="${fileMetaKeyExpr}" data-filename="${fileNameExpr}" data-filesize="${fileSizeExpr}">{{html fileExpr}}</div>' + '<div class="mck-msg-text mck-msg-content"></div>' + '</div></div>' +'<div id="mck-msg-reply-options" data-msgkey="${msgKeyExpr}"> {{html actionButtonsHtml}}</div>'+'<div class="${msgFloatExpr}-muted mck-text-light mck-text-muted mck-text-xs mck-t-xs">${createdAtTimeExpr} <span class="${statusIconExpr} mck-message-status"></span></div>' + '</div><div class="n-vis mck-context-menu">' + '<ul><li><a class="mck-message-delete">Delete</a></li></ul></div></div>';
			...
			_this.addMessage = function(msg, contact, append, scroll, appendContextMenu) {
				...
				var replyToBot=(msg.metadata&&msg.metadata.actions)?JSON.parse(msg.metadata.actions):[];
				var actionButtonsHtml="";


				replyToBot.forEach(function(item){

					if(!item.hidden){
					actionButtonsHtml = actionButtonsHtml+"<button  class= 'mck-msg-reply-button'>"+item.name+"</button>";
				}
								...
								$applozic.each(data.users, function(i, user) {
									if (typeof user.userId !== "undefined") {
										...
										if (user.connected) {
											...
											}if(user.userId=="RepoFinder"){
												MCK_LAST_SEEN_AT_MAP[user.userId] = "Online"
											}
										}
									}
								});
